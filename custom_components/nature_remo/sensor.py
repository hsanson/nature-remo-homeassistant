...
async def async_setup_entry(
    ...
    for sensor in device["newest_events"]:
        ...
        if sensor != 'mo':
            sensors.append(NatureSensor(device, sensor, api))

    if "mo" in device["newest_events"]:
        sensors.append(NatureMotionSensor(device, "mo", api))

    async_add_entities(sensors, update_before_add=True)


class NatureSensor(SensorEntity):
    """ Nature Sensor Class """

    def __init__(self, device: Dict[str, Any], sensor: str, api: NatureRemoApi):
        super().__init__()
        self.api = api
    ...
    async def async_update(self):
        """ Update sensor """
        try:
            _LOGGER.info("Update %s sensor %s", self._sensor, self._device_id)
            ...
            _LOGGER.error("Failed to update sensor - %s", error)
            self._available = False


class NatureMotionSensor(BinarySensorEntity):
    """
